...
class FrontierExplorer(Node):
    def __init__(self):
        ...
        # 用deque限制失败记录最多50条，自动丢弃最老的
        self.failed_frontiers = deque(maxlen=50)
    ...
    def map_callback(self, msg):
        self.map_data = msg
        if (self.map_timestamp is not None and 
            self.map_timestamp == msg.info.map_load_time):
            # 地图未变，直接返回，不更新
            return
    ...
    def get_nearest_frontier(self, frontiers):
        ...
        candidates = []
        failed_set = set(self.failed_frontiers) 
